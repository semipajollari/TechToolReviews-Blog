  ...
  if (!category) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-gray-50 dark:bg-gray-950">
        <h1 className="text-4xl font-black text-gray-900 dark:text-gray-50 mb-4 uppercase tracking-tighter">{t.category.notFound}</h1>
        <p className="text-gray-600 dark:text-gray-400 mb-8 font-bold">{t.category.notFoundText}</p>
        <Link to="/" className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl hover:scale-105 transition-all">{t.category.returnHome}</Link>
      </div>
    );
  }
      ...
      <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white dark:bg-gray-900 p-8 sm:p-20 rounded-[3rem] border border-gray-100 dark:border-gray-800 shadow-sm relative overflow-hidden">
          ...
          <nav className="flex items-center space-x-3 text-[10px] font-black text-indigo-600 dark:text-indigo-400 mb-10 uppercase tracking-[0.3em]">
            ...
            <span className="text-gray-400 dark:text-gray-600">{t.category.database}</span>
            <i className="fas fa-chevron-right text-[8px] text-gray-400"></i>
            <span className="text-gray-900 dark:text-gray-200">{t.category.sector} {category.id.slice(0,3)}</span>
          </nav>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-16">
          <div className="flex items-center space-x-4">
             ...
             <h2 className="text-xl font-black text-gray-950 dark:text-gray-50 uppercase tracking-tight">{t.category.activeBenchmarks}</h2>
          </div>
          <span className="bg-gray-950 dark:bg-indigo-600 text-white px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-lg">
            {category.articles.length} {t.category.reviewsVerified}
          </span>
        </div>
        ...
        {category.articles.length > 0 ? (
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[3rem] p-16 sm:p-32 text-center border border-gray-100 dark:border-gray-800 shadow-2xl relative overflow-hidden group">
            ...
            <h3 className="text-3xl sm:text-4xl font-black text-gray-900 dark:text-gray-50 mb-6 tracking-tighter relative z-10">{t.category.pendingTitle}</h3>
            <p className="text-xl text-gray-500 dark:text-gray-400 max-w-xl mx-auto font-bold leading-relaxed relative z-10">
              {t.category.pendingText}
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 relative z-10">
            {CATEGORIES.filter(c => c.id !== category.id).map(other => (
              ...
              >
                ...
                <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">{t.category.exploreDatabase} <i className="fas fa-arrow-right ml-2 text-[10px] group-hover:translate-x-2 transition-transform"></i></span>
              </Link>
            ))}
          </div>
        </div>
